...
def procesar_todo_el_sistema(ruta_hist, ruta_amb, ruta_hosp):
    ...
    # ¡ESTA LÍNEA ELIMINA LAS COMILLAS EXTRAÑAS DE EXCEL DE LAS COLUMNAS!
    ...
    # Buscador flexible de la columna de coordenadas para evitar variaciones de caracteres
    col_coordenadas = [c for c in df_hosp.columns if 'COORDENADAS' in c][0]
    
    # Separar latitud y longitud limpiando cualquier comilla interna en las celdas
    ...
    # Forzar conversión numérica limpia
...
if df_modelo is None:
    ...
    with col_analisis:
        ...
        if localidad_foco != "📍 MOSTRAR TODAS LAS LOCALIDADES":
            ...
            # Diagnóstico avanzado del contraste de recursos
        ...
        # =========================================================================
        # INYECCIÓN EXCLUSIVA DE LA TABLA MATRIZ (Ajustada a tu DF actual)
        # =========================================================================
        st.divider()
    ...
    with col_mapa:
        ...
        for idx, row_hosp in df_hospitales.iterrows():
            ...
            if pd.notna(lat) and pd.notna(lon):
                # Usar nombres de columnas insensibles a mayúsculas o espacios
